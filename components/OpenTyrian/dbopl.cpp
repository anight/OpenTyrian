/*
 * DBOPL - the DOSBox OPL emulator, as picotyrian carries it.
 *
 * This is picopop's copy, unchanged below this note so the two can be diffed:
 * taken from dosbox-x (src/hardware/dbopl.{cpp,h}), GPLv2-or-later, and trimmed
 * to the emulator core - DOSBox's DBOPL::Handler, its mixer glue, port-I/O
 * decoding and save-state serialisation are gone, and only the typedefs remain,
 * in dbopl_types.h.  PICOPOP_DBOPL_CONST_TABLES keeps picopop's name for the
 * same reason.
 *
 * It replaces the ADLIBEMU-derived emulator OpenTyrian shipped with, which did
 * its envelope arithmetic in double - soft-float on a single-precision FPU.
 * picopop measured DBOPL at 938 instructions per output sample, all integer.
 * dbopl_adapter.cpp presents it through opl.h, which is what the music player
 * calls.
 */
/*
...
/*
 * MulTable and WaveTable are the only reason this file needs a maths library,
 * and they hold the same 8,960 bytes on every boot. With
 * PICOPOP_DBOPL_CONST_TABLES they are generated at build time by
 * tools/assets/dbopl_tables.c and InitTables() copies them in, so pow() and
 * sin() have no caller left here. Without it the file behaves exactly as
 * upstream, which is what the desktop build and the dbopl-tables test rely on.
 *
 * They are COPIED rather than read where they lie, which looks like a waste of
 * 8,960 bytes of RAM until you measure it. Reading them from flash costs the
 * mixer about six points of its block budget - 24-25% average becomes 30-31% -
 * because the RP2350's XIP cache is 8 KB and WaveTable alone is 8 KB, so a
 * table indexed once per sample per operator evicts everything else on the way
 * past. RAM is not the scarce resource here and mixer headroom is; the .bss was
 * never the reason for doing this.
 */
#ifdef PICOPOP_DBOPL_CONST_TABLES
#include "dbopl_tables.inc"   /* MulTableFlash, WaveTableFlash */
#endif
...
void InitTables( void ) {
...
#endif
#if defined( PICOPOP_DBOPL_CONST_TABLES )
	//Built at build time; copy them into RAM. See the note above the include.
	memcpy( MulTable,  MulTableFlash,  sizeof MulTable );
	memcpy( WaveTable, WaveTableFlash, sizeof WaveTable );
#elif ( DBOPL_WAVE == WAVE_TABLEMUL )
	...
	&& !defined( PICOPOP_DBOPL_CONST_TABLES )
	for ( int i = 0; i < 256; i++ ) {
		//Fill silence gaps
...
/* DOSBox's Handler methods and save-state code removed. */

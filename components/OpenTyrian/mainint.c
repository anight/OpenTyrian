...
void JE_helpSystem( JE_byte startTopic )
{
	...
	do
	{
		...
		switch (page)
		{
			case 0:
				...
				} while (!(lastkey_sym == SDL_SCANCODE_ESCAPE || lastkey_sym == SDL_SCANCODE_RETURN));

				if (lastkey_sym == SDL_SCANCODE_RETURN)
				{
			...
			if (newmouse)
			{
				switch (lastmouse_but)
				{
					case SDL_BUTTON_LEFT:
						lastkey_sym = SDL_SCANCODE_RIGHT;
						break;
					case SDL_BUTTON_RIGHT:
						lastkey_sym = SDL_SCANCODE_LEFT;
						break;
					case SDL_BUTTON_MIDDLE:
						lastkey_sym = SDL_SCANCODE_ESCAPE;
						break;
				}
				do
				{
	...
	} while (lastkey_sym != SDL_SCANCODE_ESCAPE);
}
...
void JE_mainKeyboardInput( void )
{
	...
	if (keysactive[SDL_SCANCODE_BACKSPACE])
	{
		...
		// if (keysactive[SDL_SCANCODE_NUMLOCK])
		// {
...
void JE_pauseGame( void )
{
	...
	do
	{
		...
		if ((newkey && lastkey_sym != SDL_SCANCODE_LCTRL && lastkey_sym != SDL_SCANCODE_RCTRL && lastkey_sym != SDL_SCANCODE_LALT && lastkey_sym != SDL_SCANCODE_RALT)

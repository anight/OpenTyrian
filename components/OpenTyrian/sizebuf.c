...
void MSG_WriteWord(sizebuf_t * sz, unsigned int value)
{
	...
	*((Uint16 *)(sz->data + sz->bufferPos)) = SDL_Swap16LE( ((Uint16)value) );
	sz->bufferPos += 2;
}
void MSG_WriteDWord(sizebuf_t * sz, unsigned int value)
{
	...
	*((Uint32 *)(sz->data + sz->bufferPos)) = SDL_Swap32LE( ((Uint32)value) );
	sz->bufferPos += 4;
}
...
unsigned int MSG_ReadDWord(sizebuf_t * sz)
{
	...
	ret = SDL_Swap32LE(*((Uint32 *)(sz->data + sz->bufferPos)));
	sz->bufferPos += 4;
